...
// ProtocolPublisher — use case публикации протоколов манифеста под версией;
// интерфейс держим в контроллере для тестируемости команды.
type ProtocolPublisher interface {
	Execute(ctx context.Context, in usecases.PublishProtocolInput) (*entities.ProtocolPublishReport, error)
}
