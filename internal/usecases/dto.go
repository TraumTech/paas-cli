...
type FetchProtocolInput struct {
	...
	Destination string
	// Methods — HTTP-паттерны методов ("МЕТОД /путь"), которые нужно оставить в
	// контракте. Пусто — получаем контракт целиком; иначе платформа обрезает
	// контракт до них ещё до отдачи (CLI-09).
	Methods []string
}
